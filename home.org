...
**** shell layer in emacs

In order to set the default terminal emulator in emacs to vterm, the following
packages need to be on the system. The ~libvterm~ package also needs to be
installed, but it is installed as part of Linux packages for Linux systems, and
using homebrew for macOS.

#+NAME: shell-emacs-packages
#+begin_src nix
  cmake
  libtool
#+end_src
  ...
  aerospace = {
    ...
    settings = {
      ...
      mode.service.binding = {
        ...
        alt-n = [ "move-node-to-workspace --focus-follows-window --wrap-around next" "mode main" ];
        alt-p = [ "move-node-to-workspace --focus-follows-window --wrap-around prev" "mode main" ];

        alt-shift-n = [ "move-node-to-monitor --focus-follows-window --wrap-around next" "mode main" ];
        alt-shift-p = [ "move-node-to-monitor --focus-follows-window --wrap-around prev" "mode main" ];

        ctrl-alt-shift-n = [ "move-workspace-to-monitor --wrap-around next" "mode main" ];
        ctrl-alt-shift-p = [ "move-workspace-to-monitor --wrap-around prev" "mode main" ];
      };
...
#+begin_src nix
  firefox = {
    ...
    profiles.diana = {
      extensions = {
        force = true;
        packages = with pkgs.nur.repos.rycee.firefox-addons; [ bitwarden ublock-origin ];
      };
      settings = {
...
There is no ~enableBashIntegration~ setting.
